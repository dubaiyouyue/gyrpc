...
var jxszd = ["address-s4", "address-s5", "address-s6"];
var opt0 = ["省份", "地级市", "市、县级市、县"];
function setup() {
    for (i = 0; i < s.length - 1; i++){
        document.getElementById(s[i]).onchange = new Function("change(" + (i + 1) + ");promptinfo();");
		document.getElementById(jxszd[i]).onchange = new Function("changey(" + (i + 1) + ");promptinfo();");
	}
    change(0);
	changey(0);
	//初始化选择变量默认值级联
	opt0 = ["省份", "地级市", "市、县级市、县"];
	opt01 = ["省份", "地级市", "市、县级市、县"];
}
function changey(v) {
    var str = "0";
    for (i = 0; i < v; i++) { str += ("_" + (document.getElementById(jxszd[i]).selectedIndex - 1));}
    ;
    var ss = document.getElementById(jxszd[v]);
    with (ss) {
        length = 0;
        options[0] = new Option(opt01[v], opt01[v]);
        if (v && document.getElementById(jxszd[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for (i = 0; i < ar.length; i++)options[length] = new Option(ar[i], ar[i]);
                if (v)options[0].selected = true;
