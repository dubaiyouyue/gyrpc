...
;(function() {function a(a){var b=a.length;if(!(2>b)){r.comboSyntax&&(t=r.comboSyntax),r.comboMaxLength&&(u=r.comboMaxLength),r.comboTimeStamp&&(o=r.comboTimeStamp),n=r.comboExcludes;for(var d=[],e=0;b>e;e++){var f=a[e];if(!s[f]){var h=p.get(f);h.status<q&&!l(f)&&!m(f)&&d.push(f)}}d.length>1&&g(c(d))}}function b(a){var b=s[a.uri]||a.uri;a.requestUri=o?b+"?t="+o+".js":b}function c(a){return e(d(a))}function d(a){for(var b={__KEYS:[]},c=0,d=a.length;d>c;c++)for(var e=a[c].replace("://","__").split("/"),f=b,g=0,h=e.length;h>g;g++){var i=e[g];f[i]||(f[i]={__KEYS:[]},f.__KEYS.push(i)),f=f[i]}return b}function e(a){for(var b=[],c=a.__KEYS,d=0,e=c.length;e>d;d++){for(var g=c[d],h=g,i=a[g],j=i.__KEYS;1===j.length;)h+="/"+j[0],i=i[j[0]],j=i.__KEYS;j.length&&b.push([h.replace("__","://"),f(i)])}return b}function f(a){for(var b=[],c=a.__KEYS,d=0,e=c.length;e>d;d++){var g=c[d],h=f(a[g]),i=h.length;if(i)for(var j=0;i>j;j++)b.push(g+"/"+h[j]);else b.push(g)}return b}function g(a){for(var b=0,c=a.length;c>b;b++)for(var d=a[b],e=d[0]+"/",f=j(d[1]),g=0,i=f.length;i>g;g++)h(e,f[g]);return s}function h(a,b){var c=a+t[0]+b.join(t[1]),d=c.length>u;if(b.length>1&&d){var e=i(b,u-(a+t[0]).length);h(a,e[0]),h(a,e[1])}else{if(d)throw new Error("The combo url is too long: "+c);for(var f=0,g=b.length;g>f;f++)s[a+b[f]]=c}}function i(a,b){for(var c=t[1],d=a[0],e=1,f=a.length;f>e;e++)if(d+=c+a[e],d.length>b)return[a.splice(0,e),a]}function j(a){for(var b=[],c={},d=0,e=a.length;e>d;d++){var f=a[d],g=k(f);g&&(c[g]||(c[g]=[])).push(f)}for(var h in c)c.hasOwnProperty(h)&&b.push(c[h]);return b}function k(a){var b=a.lastIndexOf(".");return b>=0?a.substring(b):""}function l(a){return n?n.test?n.test(a):n(a):void 0}function m(a){var b=r.comboSyntax||["??",","],c=b[0],d=b[1];return c&&a.indexOf(c)>0||d&&a.indexOf(d)>0}var n,o,p=seajs.Module,q=p.STATUS.FETCHING,r=seajs.data,s=r.comboHash={},t=["??",","],u=2e3;if(seajs.on("load",a),seajs.on("fetch",b),r.test){var v=seajs.test||(seajs.test={});v.uris2paths=c,v.paths2hash=g}define("seajs/seajs-combo/1.0.1/seajs-combo", [], {})})();
//首页焦点图广告
;function changeFocus(id,ele){
	var adEle = $("#adsame-"+id);
	if(adEle.length > 0){
		ele.attr('isad',1).find(".title-mask").remove();
		
		var adUrl = adEle.find("a:eq(0)").attr("href")?adEle.find("a:eq(0)").attr("href"):"javascript:null";
		var imgUrl = adEle.find("img:eq(0)").attr("src");
		
		var targetA =ele.find("a:eq(0)");
		var targetImg = ele.find("img:eq(0)");
			
		targetA.attr("href",adUrl);


		targetA.attr("title",'');
		
		targetImg.attr("src",imgUrl).attr("data-original",imgUrl).attr("alt","").attr("title","");
...
//频道页焦点图广告
function changeChannelFocus(id,ele,index){
	var adEle = $("#adsame-"+id);
	if(adEle.length > 0){
		var adUrl = adEle.find("a:eq(0)").attr("href")?adEle.find("a:eq(0)").attr("href"):"javascript:null";
		var imgUrl = adEle.find("img:eq(0)").attr("src");

		var targetC = ele.find("ul.slider>li:eq("+index+")");
		targetC.find(".title-mask").remove();
		targetC.find("a").attr("href",adUrl).attr("title","");


		//targetC.find("img").attr("src",imgUrl).attr("data-original",imgUrl).attr("alt","").attr("title","");

		targetC.attr('isad',"1");

		var targetT = ele.find(".triggers>div:eq("+index+")");
		targetT.attr('isad',"1");
		targetT.find("img").attr("src",imgUrl).css({"margin-left":"-18px"}).attr("alt","").attr("title","");



		//console.log(targetT.html());
	}
}

//频道页焦点图广告 for Fashion Channel
function changeChannelFocus_fashion(id,ele){
	var adEle = $("#adsame-"+id);
	if(adEle.length > 0){
		var adUrl = adEle.find("a:eq(0)").attr("href")?adEle.find("a:eq(0)").attr("href"):"javascript:null";
		var imgUrl = adEle.find("img:eq(0)").attr("src");
		ele.find("a").attr("href",adUrl).attr("title","");
		//ele.find("img").attr("src",imgUrl).attr("data-original",imgUrl).attr("alt","").attr("title","");
		ele.attr("isad","1");
	}
...
//频道页专题sky广告
function changeFeatureAd(id,ele){
	var adEle = $("#adsame-"+id);
	if(adEle.length > 0){
		var adUrl = adEle.find("a:eq(0)").attr("href")?adEle.find("a:eq(0)").attr("href"):"javascript:null";
		var imgUrl = adEle.find("img:eq(0)").attr("src");
		...
		ele.find("a").attr("href",adUrl).attr("title","");
		ele.find("img").attr("src",imgUrl).attr("title","").attr("alt","").attr("data-original",imgUrl);

	}
}

//vogueTV广告
function changeTV(id,ele){
	var adEle = $("#adsame-"+id);
	if(adEle.length > 0){
		
		var adUrl = adEle.find("a:eq(0)").attr("href")?adEle.find("a:eq(0)").attr("href"):"javascript:null";
		var imgUrl = adEle.find("img:eq(0)").attr("src");
		var adTitle = adEle.find("img:eq(0)").attr("alt");
		

		var targetA =ele.find("a:eq(0)");
		var targetImg = ele.find("img:eq(0)");
		var targetInnerA = ele.find(".title>a");

		targetA.attr("href",adUrl).attr("title",'');
		
		targetImg.attr("src",imgUrl).attr("data-original",imgUrl).attr("alt",'').attr("title","");

		targetInnerA.attr("href",adUrl).attr("title",'').attr("alt",'').html("<em></em>"+adTitle);
...
//重点推荐文字链广告
function changeRecTextAd(ids,ele){
	//ele : .slide-ads>div>.sub-links
	var adIdArr = ids.split(",");
	var adsAele = ele.find("a");

	for(var i=0; i<adIdArr.length; i++){
		if ($("#adsame-"+adIdArr[i]).length > 0) {
			var adEle = $("#adsame-"+adIdArr[i]);

			var trgA = adsAele.eq(i);
			trgA.html(adEle.text());
			trgA.attr("href",adEle.attr("href"));
			trgA.attr("title",adEle.text());

			ele.parent().parent().attr("rel",i);//传递给ads.js广告的数量
			
			/*
			//原处理方式：2014/10/24 修改
			//8个文字链分布在4个不同的div里，需要谨慎处理
			var Cele = ele.eq(parseInt(i/2)).find("a:eq("+(i%2)+")");

			if(Cele.length > 0) {
				Cele.html(adEle.text());
				Cele.attr("href",adEle.attr("href"));
				Cele.attr("title",adEle.text());
			} else {
				ele.eq(parseInt(i/2)).append( "<a href='"+adEle.attr("href")+"' target='_blank'>"+adEle.text()+"</a>");
			}
			*/
