...
class OkoidAction extends BaseAction {
    ...
    public function index() {
		...
		$list=M('dingdan')->where('userid='.$uid.' and ((fk=1 and sh=1 and tstatus=1) or (fk=0 and tstatus=2) or tuihuo=2)')->limit($lm,$m)->order('id desc')->select();
		...
		if($psffd>1){
			if(empty($list)) exit;
			$this->display('./GZphp/Tpl/Home/Default/Okoid_ajax.html');
		}
		else $this->display();
	}
